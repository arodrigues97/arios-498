...
public final class VisualCommand extends CommandPlugin {
	...
	case "interface":
	    ...
	    player.getInterfaceManager().open(new Component(componentId));
	    return true;
	case "iconfig":
	case "inter_config":
	    if (args.length < 2) {
	...
	case "config_loop":
		...
		player.getConfigManager().send(i, value);
	    }
	    return true;
	case "string":
	    if (args.length < 3) {
	...
	/*case "tg":
		...
		for (RegionChunk chunk : chunks) {
			...
			for (int y = 0; y < RegionChunk.SIZE; y++) {
			    GroundItemManager.create(new Item(4151), chunk.getCurrentBase().transform(x, y, 0), player);
			}
		    }
		}
	    }
